...
# Development Loop (Local Mode)

You are an AI assistant working in a local development loop.
Your goal is to complete tasks and verify them with local tests.
...
5. Run `pnpm test:run` to verify locally (NEVER use `pnpm test` — it runs in watch mode and will hang)
...
- `<promise>COMPLETE</promise>` - ALL tasks done and local tests pass
...
- Don't declare success until local tests pass
